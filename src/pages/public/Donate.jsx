    ...
    <div>
      <section className="border-b border-border bg-primary text-white">
        <div className="container-site py-12">
          ...
          <h1 className="mt-2 font-serif text-3xl sm:text-4xl font-bold text-white">Help us reach every voter before February 2027</h1>
          <p className="mt-3 max-w-2xl text-white/75">
            Your contribution goes straight to field organizing, outreach, phone banks, and the day-to-day work of a people-powered campaign. Small-dollar donations are the backbone — thank you for being part of it.
          </p>
      ...
      <section className="container-site py-10 grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2">
          <form onSubmit={submit} className="space-y-8">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-5 gap-2" role="radiogroup">
                  {presets.map((p) => (
                    <label key={p} className={`flex cursor-pointer items-center justify-center rounded-sm border px-3 py-3 text-center ${!custom && amount === p ? 'border-accent bg-accent-tint ring-1 ring-accent' : 'border-border bg-surface hover:border-accent/60'}`}>
                      ...
                      <span className={`font-semibold ${!custom && amount === p ? 'text-[#1C2430]' : 'text-ink-muted'}`}>₦{p.toLocaleString()}</span>
                    </label>
                  ))}
                </div>
                <div className="mt-3">
                  ...
                  <div className="relative">
                    ...
                    <input id="custom-amount" inputMode="numeric" value={custom} onChange={(e) => { setCustom(e.target.value.replace(/[^\d.]/g, '')); setAmount(0) }}
                      placeholder="5000.00" className="w-40 rounded-sm border border-border bg-surface py-2 pl-7 pr-3 text-sm focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary" />
                  </div>
                </div>
              </fieldset>
            ...
            <Card className="p-6">
              ...
              <p className="mt-1 text-sm text-ink-muted">Nigerian electoral law requires us to collect and report the name, address, and contact details of every contributor. We collect them on every contribution to protect you and to keep our records accurate.</p>
              <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="sm:col-span-2">
                  ...
                  <div>
                    <label htmlFor="d-city" className="mb-1 block text-sm font-medium text-ink">City / Town</label>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div>
                      <label htmlFor="d-zip" className="mb-1 block text-sm font-medium text-ink">Postal code</label>
        ...
        <div className="space-y-4">
          <Card className="p-5">
            <h2 className="font-serif text-base font-bold text-primary">Contribution rules</h2>
            <ul className="mt-3 space-y-2 text-sm text-ink-muted">
              <li>• Contributions are governed by Nigerian electoral law and INEC regulations.</li>
              <li>• Contributions must come from Nigerian citizens or lawful residents.</li>
              <li>• Contributions from foreign nationals and government-owned entities are prohibited.</li>
            </ul>
          </Card>
          <Card className="p-5">
          ...
          <div className="rounded border border-border bg-surface-2 p-5 text-xs text-ink-muted">
            <p className="font-semibold text-ink">Paid for by the Federal Unity Party.</p>
