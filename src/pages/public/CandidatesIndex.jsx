    ...
    <div>
      <section className="border-b border-border bg-primary text-white">
        <div className="container-site py-12">
          ...
          <h1 className="mt-2 font-serif text-3xl sm:text-4xl font-bold text-white">Our 2026 slate</h1>
          <p className="mt-3 max-w-2xl text-white/75">
            From the state senate to the city council, these candidates committed to our written platform — and to the accountable, people-powered way we run.
          </p>
        </div>
      </section>
          ...
          {seed.candidates.map((c) => (
            <Link key={c.slug} to={`/candidates/${c.slug}`} className="group">
              <Card className="flex gap-5 p-5 transition-colors hover:border-primary/40">
                <div aria-hidden="true" className="hidden sm:flex h-16 w-16 flex-shrink-0 items-center justify-center rounded-full bg-primary text-lg font-bold text-white">
                  {c.photo}
                </div>
                <div className="min-w-0">
                  ...
                  <p className="mt-2 text-sm font-semibold text-accent">View profile and donate</p>
                </div>
              </Card>
            </Link>
          ))}
